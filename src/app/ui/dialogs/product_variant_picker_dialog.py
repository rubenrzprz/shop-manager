...
class ProductVariantPickerDialog(QDialog):
    def __init__(self, parent=None) -> None:
        ...
        self.setWindowTitle("Select Product Variant")
        self.resize(840, 460)

        self._search_input = QLineEdit()
        self._search_input.setPlaceholderText("Search by product, SKU, variant, size, or color")
        self._search_input.textChanged.connect(self._apply_filter)

        self._table = QTableWidget()
        self._table.setColumnCount(8)
        self._table.setHorizontalHeaderLabels(
            ["ID", "Product", "SKU", "Variant", "Size", "Color", "Price", "Status"]
        )
        ...
        layout = QVBoxLayout()
        layout.addWidget(QLabel("Product Variant"))
    ...
    def _load_variants(self) -> None:
        try:
            ...
            QMessageBox.critical(self, "Could not load product variants", str(exc))
            return
    ...
    def _apply_filter(self) -> None:
        ...
        variants = [
            ...
            if self._matches_variant(variant, query)
        ]
    ...
    def _accept_selected_variant(self) -> None:
        ...
        if variant is None:
            QMessageBox.information(self, "No product variant selected", "Select a product variant.")
            return
